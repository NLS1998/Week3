...
const fs = require('fs');
nodesTxtFile = fs.readFileSync('nodes.txt'); nodes = JSON.parse(nodesTxtFile);
//setup zeromq
...
sock.send(["status", myhostname + "=alive"]); }, 500);
//for each key value in nodes 
Object.entries(nodes).forEach(([hostname,ip]) => {
//print the hostname IP
console.log("hostname = " + hostname + " ip = " + ip); //create a number of subscribers to connect to publishers
var subsockets = []; if(myhostname != hostname ){
tempsoc = zmq.socket("sub");
tempsoc.connect("tcp://" + ip + ":3000"); tempsoc.subscribe("status");
console.log("Subscriber connected to port 3000 of " + hostname); tempsoc.on("message", function(topic, message) {
console.log(
"received a message from " + hostname + " related to:", topic.toString("utf-8"),
"containing message:",
